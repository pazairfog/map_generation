import {ascending, descending, max, mean, min, quantile, scan} from 'd3-array'
...
import {makeName, makeRandomLanguage} from './language.js'
...
export function customVoronoi(pts, extent) {
  extent = extent || defaultExtent;
  const w = extent.width / 2;
  const h = extent.height / 2;
  const delaunay = Delaunay.from(pts);
  // @TODO Replace voronoi by Delaunay.voronoi (much faster)
  // @see https://twitter.com/mbostock/status/976537438121205760
...
export function makeMesh(pts, extent) {
  extent = extent || defaultExtent;
  var vor = customVoronoi(pts, extent);
  var vxs = [];
  var vxids = {};
  var adj = [];
  var edges = [];
...
export function customQuantile(h, q) {
  var sortedh = [];
  for (var i = 0; i < h.length; i++) {
    sortedh[i] = h[i];
  }
  sortedh.sort(ascending);
  return quantile(sortedh, q);
}
...
export function pike(mesh, slope) {
...
export function forceBorders(mesh, slope) {
  var boundary = 0.40;
  return mesh.map(function(x) {
...
export function relaxOnBorder(h) {
  var threshold = 0.45;
...
export function mountainsCentered(mesh, n, d, r) {
  r = r || 0.05;
  var mounts = [];
  for (var i = 0; i < n; i++) {
...
export function getTerritories(render) {
  ...
  var n = render.params.nterrs > render.cities.length ? render.cities.length : render.params.nterrs;
  var flux = getFlux(h);
  var terr = [];
  var queue = new FlatQueue();
  ...
  while (queue.length) {
    ...
    var u = queue.peekValue();
    // Remove the top item (index and value).
    queue.pop();
...
export function visualizeVoronoi(svg, field, lo, hi) {
  ...
  svg.selectAll('path.field')
    ...
    .style('fill', function(d, i) {
      return interpolateViridis(mappedvals[i]);
    });
}

export function visualizeVoronoiAsTerrain(svg, field) {
  var tris = svg.selectAll('path.field').data(field.mesh.tris)
  tris.enter()
    .append('path')
    .classed('field', true);
...
export function visualizeTerrain(svg, render) {
  ...
  var lo = 0;
  var hi = max(render.h);
  var mappedvals = render.h.map(function(x) {
    return x > hi ? 1 : x > lo ? (x - lo) / (-hi + lo) : (x - lo) / (hi - lo)
  });
  var color = scaleLinear()
    .domain([0, 0.5])
    .interpolate(interpolateHcl)
    .range([rgb("#c9ae7b"), rgb('#ffffff')]);
...
export function drawPaths(svg, cls, paths, color, strokeSize) {
  color = color || '#000000';
...
export function renderTerrain(svg, render) {
  render.rivers = getRivers(render.h, 0.01);
  render.coasts = contour(render.h, 0);
  render.terr = getTerritories(render);
  render.borders = getBorders(render);
...
export function drawTerrain(svg, render) {
  drawPaths(svg, 'river', render.rivers);
  drawPaths(svg, 'coast', render.coasts);
  drawPaths(svg, 'border', render.borders);
  visualizeSlopes(svg, render);
...
export function doTerrain(svg, params) {
  var render = {
    params: params
  };
  var width = svg.attr('width');
